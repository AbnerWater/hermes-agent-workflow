  ...
  return (
    <div className="rounded-2xl border border-border/65 bg-[color-mix(in_srgb,var(--dt-card)_70%,transparent)] py-0.5 shadow-[0_0_0_1px_color-mix(in_srgb,var(--dt-card)_30%,transparent)_inset]">
      ...
      >
        ...
        <span className="truncate">{copy.queuedCount(entries.length)}</span>
      </button>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[0.73rem] leading-4 text-foreground/92">
                    {entry.text.trim() || (entry.attachments.length > 0 ? copy.attachmentOnlyTurn : copy.emptyTurn)}
                  </p>
                  {(attachmentsCount > 0 || isEditing) && (
                    <div className="mt-0.5 flex items-center gap-1.5 text-[0.64rem] text-muted-foreground/75">
                      {attachmentsCount > 0 && <span>{copy.attachmentCount(attachmentsCount)}</span>}
                      {isEditing && (
                        <span className="text-[color-mix(in_srgb,var(--dt-composer-ring)_78%,var(--muted-foreground))]">
                          {copy.editingInComposer}
                        </span>
                      )}
                    </div>
                  )}
                ...
                >
                  <Tip label={copy.editQueuedTurn}>
                    <Button
                      aria-label={copy.editQueuedTurn}
                  ...
                  <Tip label={copy.sendQueuedTurnNow}>
                    <Button
                      aria-label={copy.sendQueuedTurnNow}
                      className="h-5 w-5 rounded-md"
                  ...
                  <Tip label={copy.deleteQueuedTurn}>
                    <Button
                      aria-label={copy.deleteQueuedTurn}
                      className="h-5 w-5 rounded-md"
