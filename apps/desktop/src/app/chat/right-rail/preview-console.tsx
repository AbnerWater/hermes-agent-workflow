...
function ConsoleRow({ copyText, log, onSend, onToggleSelect, selected }: ConsoleRowProps) {
  ...
  return (
    ...
    >
      <Tip label={selected ? copy.deselectEntry : copy.selectEntry}>
        <button
          className={cn(
      ...
      <span className="opacity-0 transition-opacity group-hover/row:opacity-100">
        <CopyButton
          ...
          errorMessage={copy.couldNotCopyConsoleOutput}
          iconClassName="size-3"
          label={copy.copyThisEntry}
          showLabel={false}
          text={copyText}
        />
        <Tip label={copy.sendThisEntryToChat}>
          <button
            className="rounded-md p-1 text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
            onClick={onSend}
            type="button"
...
export function PreviewConsoleTitlebarIcon({ consoleState }: { consoleState: PreviewConsoleState }) {
  ...
  return (
    <>
      <PanelBottom />
      {logCount > 0 && <span className="sr-only">{copy.consoleMessages(logCount)}</span>}
    </>
  )
}
...
export function PreviewConsolePanel({
  ...
  function sendLogsToComposer(entries: ConsoleEntry[]) {
    ...
    const block = [copy.previewConsoleBlockHeader, '```', ...entries.map(formatLogLine), '```'].join('\n')

    requestComposerInsert(block, { mode: 'block', target: 'main' })
    consoleState.clearSelection()
    notify({
      kind: 'success',
      title: copy.sentToChat,
      message: copy.logEntriesAddedToComposer(entries.length)
    })
  }
  ...
  return (
    ...
    >
      <div
        aria-label={copy.resizePreviewConsole}
      ...
      <div className="flex h-8 shrink-0 items-center justify-between border-b border-border/50 px-2">
        <div className="flex items-center gap-2 text-[0.6875rem] font-medium text-muted-foreground">
          ...
          {selectedLogIds.size > 0 && (
            <span className="rounded-full bg-muted px-1.5 py-px text-[0.5625rem] text-muted-foreground">
              {copy.selectedCount(selectedLogIds.size)}
            </span>
          )}
        </div>
        <div className="flex items-center gap-1">
          ...
          <CopyButton
            ...
            errorMessage={copy.couldNotCopyConsoleOutput}
            iconClassName="size-3"
            label={visibleSelection.length > 0 ? copy.copySelectedToClipboard : copy.copyAllToClipboard}
      ...
      >
        {logs.length > 0 ? (
          ...
          <div className="py-2 text-muted-foreground/70">{copy.noConsoleMessages}</div>
        )}
      </div>
    </div>
  )
