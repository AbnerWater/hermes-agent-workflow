...
export function McpSettings({ gateway, onConfigSaved }: McpSettingsProps) {
  ...
  useEffect(() => {
    ...
    getHermesConfigRecord()
      ...
      .catch(err => notifyError(err, copy.mcpConfigFailedToLoad))

    return () => void (cancelled = true)
  }, [copy.mcpConfigFailedToLoad])
  ...
  if (!config) {
    return <LoadingState label={copy.loadingMcpServers} />
  }
  ...
  const saveServer = async () => {
    ...
    if (!nextName) {
      notify({ kind: 'error', title: copy.nameRequired, message: copy.nameRequiredMessage })
    ...
    try {
      ...
      if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
        throw new Error(copy.mcpJsonMustBeObject)
      }

      parsed = raw as Record<string, unknown>
    } catch (err) {
      notifyError(err, copy.invalidMcpJson)
    ...
    try {
      ...
      onConfigSaved?.()
      notify({ kind: 'success', title: copy.mcpServerSaved, message: copy.mcpServerSavedMessage(nextName) })
    } catch (err) {
  ...
  const reloadMcp = async () => {
    if (!gateway) {
      notify({ kind: 'warning', title: copy.gatewayUnavailable, message: copy.reconnectGatewayReloadMcp })
    ...
    try {
      ...
      notify({ kind: 'success', title: copy.mcpToolsReloaded, message: copy.newToolSchemasFreshTurns })
    } catch (err) {
      notifyError(err, copy.mcpReloadFailed)
    } finally {
      setReloading(false)
    }
  }
    ...
    <SettingsContent>
      <div className="mb-4 flex items-center justify-end gap-4">
        ...
        <Button disabled={reloading} onClick={() => void reloadMcp()} size="xs" variant="text">
          {reloading ? copy.reloading : copy.reloadMcp}
        </Button>
      </div>
      ...
      <div className="grid min-h-0 gap-6 lg:grid-cols-[16rem_minmax(0,1fr)]">
        <div className="min-h-64">
          {names.length === 0 ? (
            <EmptyState description={copy.noMcpServersDescription} title={copy.noMcpServers} />
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-1 flex items-center gap-1.5">
                      ...
                      {server.disabled === true && <Pill>{copy.disabled}</Pill>}
                    </div>
                  </button>
                )
              })}
        ...
        <div className="grid content-start gap-3">
          <div className="flex items-center gap-2 text-sm font-medium">
            ...
            {selected ? copy.editServer : copy.newServer}
          </div>
          ...
          <label className="grid gap-1.5">
            <span className="text-xs text-muted-foreground">{copy.serverJson}</span>
            <Textarea
          ...
          <div className="flex items-center justify-between">
            ...
            <Button disabled={saving} onClick={() => void saveServer()} size="sm">
              {saving ? copy.saving : copy.saveServer}
            </Button>
          </div>
        </div>
      </div>
