...
export function ModelPickerDialog({
  ...
  const copy = appCopy.modelPicker
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent className={cn('max-h-[85vh] max-w-2xl gap-0 overflow-hidden p-0', contentClassName)}>
        <DialogHeader className="border-b border-border px-4 py-3">
          <DialogTitle>{copy.switchModel}</DialogTitle>
        ...
        <Command className="rounded-none bg-card" shouldFilter={false}>
          ...
          <CommandList className="max-h-96">
            {!loading && !error && <CommandEmpty>{copy.noModelsFound}</CommandEmpty>}
            <ModelResults
        ...
        <DialogFooter className="flex-row items-center justify-between gap-3 border-t border-border bg-card p-3 sm:justify-between">
          <label className="flex cursor-pointer select-none items-center gap-2 text-xs text-muted-foreground">
            ...
            />
            {sessionId ? copy.persistGloballySessionOnly : copy.persistGlobally}
          </label>
...
function ModelResults({
  ...
  const copy = useAppCopy().modelPicker
  ...
  if (error) {
    return (
      <div className="px-3 py-3">
        <InlineNotice kind="error" title={copy.couldNotLoadModels}>
          {error}
        </InlineNotice>
      </div>
    )
...
function ModelPrice({ price, isCurrent }: { price?: ModelPricing; isCurrent: boolean }) {
  ...
  return (
    <span
      ...
      title={copy.inputOutputPriceTitle}
    >
